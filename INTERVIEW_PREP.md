...
| OAuth redirect to `/callback` not `/dashboard` | Zustand hydrated before navigation; page refreshes with valid cookies work correctly | Extra round-trip (`/users/me`) on every OAuth sign-in | Direct `/dashboard` redirect leaves Zustand empty — `AuthGuard` would immediately bounce to `/login` even with valid cookies; the extra call is negligible |
